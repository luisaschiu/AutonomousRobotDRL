...
def aruco_angle(dictionary, origImg):
    ...
    if markerIds is not None and len(markerIds) > 0:
        for markerRect in markerCorners:
            bottomLeft = markerRect[0][3]
            bottomRight = markerRect[0][2]
            topRight = markerRect[0][1]
            topLeft = markerRect[0][0]
...
def aruco_location(dictionary, origImg):
    gray = cv2.cvtColor(origImg, cv2.COLOR_BGR2GRAY)
    detectorParams = cv2.aruco.DetectorParameters()
    detector = cv2.aruco.ArucoDetector(dictionary, detectorParams)
    markerCorners, markerIds, _ = detector.detectMarkers(gray)
...
def show_aruco_param(dictionary, origImg):
    gray = cv2.cvtColor(origImg, cv2.COLOR_BGR2GRAY)
    detectorParams = cv2.aruco.DetectorParameters()
    detector = cv2.aruco.ArucoDetector(dictionary, detectorParams)
    markerCorners, markerIds, _ = detector.detectMarkers(gray)
    ...
    if markerIds is not None and len(markerIds) > 0:
        ...
        for markerRect in markerCorners:
            ...
            # print("Bot Left: " + str(bottomLeft))
            # print("Top Right: " + str(topRight))
            # print("Bot Right: " + str(bottomRight))
            diff = (topRight[0] - topLeft[0], topLeft[1] - topRight[1]) # (x, y)
            angle = np.arctan2(diff[1], diff[0]) * 180 / np.pi
            # normalize values to 0 - 360 degrees
            if angle < 0:
                angle += 360
            width = 5
            # convert angle to degrees for processing angle in radians
            angle_deg = round(angle, 2)
            # draw position in pixels
            center = (bottomRight + topLeft) / 2
            cv2.putText(frame,
                        str(round(angle, 0)) + 'deg ',
